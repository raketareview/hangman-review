...
Название "это буква использована ранее" не добавляет ничего нового к пониманию по сравнению с "это использованная буква"
```java
...
Возможно, в данном случае здесь прекращать работу это сильно упрощает код, тогда можно оставить, как есть.  
...
Список умеет делать все то же, что и массив, и даже больше
```java
...
**8. этот метод может брать значение поля ихз окружения**
```java
...
public static void printStateOfWord() {
  for (char symbol : maskedWord) {
