...
+ 👍 Перед игрой публикуются правила
...
//ПРАВИЛЬНО:
public class Dictionary {
  private final List<String> words = new ArrayList<>();  <-- это не экземпляр класса Dictionary
  //...
}
```
...
Чтобы пользоваться твоим классом было корректно, сделай перегруженные конструкторы:
```java
public class Dictionary {
  //...  
  public Dictionary(String path, int maxWords) {
...
```java
//Создание словаря и загрузка в него слов- 1 операции
...
Преимущество последнего вью в том, что можно делать разные реализайции с тем же интерфейсом.  
...
В котором должны быть объединены состояния(текст слова и его маска) и поведение(открыть букву etc.)  
Сейчас эта сущность разделена на несколького классов, в том числе на утилиту `WordUtils`- то есть, чисто процедурный подход.
