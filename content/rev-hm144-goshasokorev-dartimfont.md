...
//ПРАВИЛЬНО:
List<Character> randomWordLetters = new ArrayList<>();
```

- Газвание методов должно быть глаголом в повелительном наклонении. Это название- существительное
```java
void mainMenu()
```
...
```
По аналогии с уже применяющимися тобой назваиями: `Set<String> mistakeLetters`

- Метод ен обновляет outputWord, он его создает
```java
