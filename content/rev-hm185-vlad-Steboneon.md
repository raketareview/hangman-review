...
И если где-то в слое вью действительно лежит класс-вью, то это выглядит не более чем совпадением, а результатом системного понимания идей этой архитектуры.
...
//ПРАВИЛЬНО:
System.out.printf("В файле нет слов длиной %d букв!", wordLength);
```
...
Сначала переводит строку в число, а потом проверяет это число на вхождение в заданные диапазон
```java
public class InputControler {
  public static boolean isNumberInRange(String input, int min, int max) {...}
}
...
Из-за аргумента-флага `languageChoice`, алгоритм в методе валидации разделается на две операции- русская и английская валидация.  
...
public class RuLetterValidator implement LetterValidator{
...
public class EnLetterValidator implement LetterValidator{
...
Ни один из них не представляет из себя никакой внятной объектно-ориентированной сущности и понять причину их существования совсем не легко.
...
Если бы это меню было простым, состояло из нескольких пунктов, то такое процедурное меню можно было бы использовать и в ООП программе.  
...
Поэтому ты должен, *по крайней мере в этой реализации*, перенести классы из других пакетов в эти три или четыре базовых слоя MVC(S).

Main это особый класс, он находится вне слоев MVC, поэтому должен находиться отдельно от них- либо просто в корне проекта, либо в своем пакете. 
