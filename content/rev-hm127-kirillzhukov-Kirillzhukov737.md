...
**0. Мусор в репозтории**. Разберись, какие файлы нужно помещать в реползиторий, а какие нет.
...
//ПРАВИЛЬНО:
System.out.printlnf("%d %s", (i + 1), playersNames.get(i));
```
...
Поэтому знание о том, что юзер выбрал себе игрока с именем "Дон Карлеонэ", остается только в памяти самого юзера, но не в созданном экземпляре Player.
...
По сути, для антуража с выбором имени, достаточно сделать простую процедурную утилиту.  
Типа такой:
```
...
Тогда вместо нескольких специализированных классов файолового чтения/записи, где имя файла  будет жестко прописано в каждом из классов, 
...
Нужно либо возвращать не оригинал списка слов, а их копию.  
...
Объекты должен создавать тот, кто их использует. В данном случае экземпляр класса `FileWriterService` должен создавать сам Словарь. 
...
private void isWordLetter(char letter) {
...
if (isWin()) {...}
```
Вспомогательные методы будут даже размером в одну строку. 
...
Даже основная игровая логика в классе GameService читается плохо- потому что находится в большом божественном методе, где за частным не видно общего.

## ВЫВОД

Стоит эту игру сделать проще, но чище с точки зрения кода.

Есть простейшие ошибки конвенции кода, стоит почитать "Oracle Java code conventions".
