...
во время работы программы, значения полей этого класса не изменяются.  
...
Зачем это всё хранить в результате, мне неясно.
...
Сообщение(message) в том контексте, как оно тут используется, может появиться только где-нибудь в контроллере или вью.  

`GameStatus statusAfter` это как следует из названия, состояние ДВИЖКА ИГРЫ- выиграл, проиграл etc.  
Да, состояние движка меняется в результате выполнения хода, но это немного другое. 
...
В этом же пакете лежит не только движок игры модель `class GameEngine`, но и его контроллеры- `class Game` и `class GameRound`.  
Это классы разных слоев. Контроллер движка игры не является движком игры и не может лежать вместе с ним в пакете `engine`.
...
Непонятно, насколько осознанно ты используешь MVC, потому что не могу понять мотивацию некоторых твоих решений.    
