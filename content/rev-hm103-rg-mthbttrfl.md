...
![pic](https://github.com/raketareview/hangman_review/blob/master/content/resources/rev-hm103/img0.png)

2. Неправильная последовательность в порядке отображения введенных букв: букву 'ц' я ввел второй, но в распечатке она стала первой. (2)
...
![pic](https://github.com/raketareview/hangman_review/blob/master/content/resources/rev-hm103/img1.png)
...
![pic](https://github.com/raketareview/hangman_review/blob/master/content/resources/rev-hm103/img2.png)

Псевдографика в консоли уместна и в ней есть даже некий шарм. 
Но в свинге есть возможность показывать нормальные картинки. 
Можно попросить чатгпт, чтобы он нагенерировал картинок хангмана и использовать их в этой игре.
...
В других средах нужно будет менять код в этом классе, чтобы вывод осуществлялся по правилам этой среды, тоесть это лишняя причина для изменения класса.
...
public char get() {
  ...
  //и вернуть ее без сохранения в классе
}
```
...
Классы, которые использующие свинг, например, наследующиеся от `JFrame`, должны только обеспечивать ввод информации от пользователя и вывод информации для ее показа пользователю.
...
Например, все, что связано с зависимостью от языка- классы внутри себя себя сами конфигурируют через цепочки if'ов и анализ выбранного языка в конфигурации
```
public class Input {
...
ООП-подход состоит в том, чтобы все зависимости формировались как можно выше. Например, в слое `main`. 
Например:
