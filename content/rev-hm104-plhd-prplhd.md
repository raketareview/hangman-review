...
- Метод открывает букву, ему безразлично- введенная ли это буква, или еще какая-то. Это просто буква, которую он откроет
```
...
Использование `System.exit(0)` для этого не совсем правильно, о чем поговорим ниже, но сам алгоритм действий "не можем прочитать слова- закрываем программу" абсолютно верный.

При этом выводится сообщение "Проверьте доступность словаря для начала игры"- а где юзер должен проверить этот словарь? 
...
Понимаю, что трудно балансировать между "разделить методы на много маленьких, каждый из которых будет делать только одно дело" и "нельзя слишком дробить логику между методами"- здесь нужно нащупать баланс.
