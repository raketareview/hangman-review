...
3. Команды работают неправильно- здесь программа должна принять или "y", или "n". Любая другая команда является неправильной
```
...
+ 👍 Молодец, что выявил сущность "отгадываемое слово + маска + методы для работы с ними" и сделал из него класс. 
Значит ООП в проекте уже есть. 
...
+ 👍 Принимает в конструктор нужные зависимости- это хорошо
```
Game(MaskedWord maskedWord, TextReader input)
```

- "Уровень сложности"- это отдельная сущность, которая живет в программе. В данном случае нужно сделать ее, для начала, перечислением
```
private const int Easy = 1;
private const int Medium = 2;
private const int Hard = 3;
