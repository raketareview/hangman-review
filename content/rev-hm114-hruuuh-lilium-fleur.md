...
```
Как назвать? 
`String getRandom()`- не подходит- гетеры возвращают без удаления.  
...
- Это класс для использования в многопоточной среде. Здесь многопоточности не замечено, достаточно простого класса Random
```
...
Все вместе это не читается, вводи поясняющие переменные.
...
С точки зрения ООП все ок. Есть грамотное разделение программы на классы.
