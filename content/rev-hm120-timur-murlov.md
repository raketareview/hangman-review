...
public class GameInterface {
  public int getUserChoice() {
    //...
    while (number != 1 && number != 2)   <-- 2 это СТСТАРТ_ИГРЫАРТ
    //...
  }
}
```
...
- При попытке открыть букву, которой нет в слове, нужно бросать исключение
```
void updateHiddenVersion(char letter)
```
...
3 неполных раза до ближайшей отгадываемой буквы и один полный- от ночала до конца, чтобы убедиться, что таких букв больше нет.
