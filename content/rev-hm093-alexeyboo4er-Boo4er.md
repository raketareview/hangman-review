...
Любую другую команду программа принимать тут не должна
```
...
Для учебы в любом случае придется познакомится с файловым чтением. Виселица хороший повод для этого.
...
то при твоем подходе каждый из этих экземпляров будет дублировать в памяти миллион слов твоего словаря.
...
//ПРАВИЛЬНО:
public static void printStage(int pictureNumber) {
  System.out.println(HANGMAN_STAGES[pictureNumber]);
}
```
...
- Нарушение конвенции кода. Вспомогательные приватные методы располагаются выше публичных.
...
private static boolean updateHiddenWord(String secretWord, char[] hiddenWord, char guess) {
  //...  
  hiddenWord[i] = guess; <-- ВЫПОЛНЯЕТ КОММАНДУ, открывает букву в слове
  //...
...
Здесь ООП нет. Думать, что для ООП достаточно как-то разделить программу на несколько классов- опасное заблуждение.
...
Если изменяемые данные(то есть не константы) являются в классе статическими полями, значит в проекте невозможно создать несколько корректно работающих экземпляров классов(объектов). 
...
Поэтому пока не разобрался с ООП, лучше пока вообще не делай статические методы в классах.
...
Для понимания основных различий между процедурным и ООП стилями, посмотреть стрим по игрре "Крестики-нолики".  
