...
Альтернативный вариант: класс должен принимать в конструктор также значение длины слова и регулярное выражение для его фильтрации.

**6. class InputHandler**, валидированный ввод буквы + хранилище введенных букв

- Переменная хранит не дубликаты букв, а уникальные буквы. Иное в принципе тут невозможно, потому что буквы хранятся в Set, что гарантирует их уникальность в этой коллекции
```
Set<String> duplicateLettersSet;
```
...
В данном случае класс получает список слов из WordsListFromFile и выбирает из него одно случайное. Это что-то типа WordRepository.

+ 👍 В принципе, идея разделения на два класса процессы чтения слов и выдачи из них одного случайного слова это хорошая идея.  
