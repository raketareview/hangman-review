...
**2. Нарушение DRY**, магические буквы, числа, слова. Вводи константы (1)
```
System.out.println("""
  1 - Начать новую игру
  0 - Выйти из приложения
...
Эта игра состоит из одинаковой последовательности действий от начала до конца: вводить буквы до тех пор, пока не выиграл или не проиграл.

"Стратегия" там, где одинаковые действия на разных этапах должны приводить к разным результатам.

Классический пример использования паттерна- калькулятор на Swing или Android.  
...
Импользовать любой паттерн нужно не потому что можешь, а для того, чтобы упростить алгоритм и сделать его более понятным.
...
- Енам не должен так называться, потому возникает путаница в концепциях: "Состояние" как паттерн в игре и "Состояние", как графическое изображение уровня ошибки
```
enum HangmanStates
...
Маска ведь это ведь просто набор данных, которые сами по себе не зависят от способа их отображения.

Короче, все это полная ерунда, не наводи тень на плетень. 
Вот нормальный интерфейс `Renderer` под твою реализацию:
```
public interface Renderer {
  void render(String picture, List<Character> maskedWord);
}
```
...
Но в своей работе `InputHandler` все равно зависит от представления, потому что и печатает и общается с юзером через консоль:
```
...
Если лень своим ходом переосмысливать ООП, можешь обратиться к Сергею за расширенными матриалами к Хангману. 
