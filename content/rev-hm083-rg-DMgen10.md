...
**11. Если есть isWin()**, то сделай `isLose()` и `isGameOver()` заодно
```
while (true) {
  //процесс игры
  if (attempts == 0) {
...
**14. Выноси main в отдельный класс**, если внутри main'a создаешь экземпляр класса, в котором и находится этот main
```
