...
**4. При форматированном выводе** перенос строки отделяй пробелом. Результат будет тот же, но шаблон будет лучше читаться
```
...
**5. Приоритет- читаемость кода**, а не экономия строчек
...
**9. Вместо создания промежуточных переменных состояний игры**, сделай методы, которые будут определять состояния игры
```
