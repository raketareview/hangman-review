...
- Если в аргументы метода пришло некорректное значение `hp`(hp > 6 или hp < 0), то это баг алгоритма и нужно бросать исключение, чтобы сразу же выявить этот баг и устранить его.
