...
Все это приводит к ненужным сложностям. Намного проще использовать стандартный подход, при котором для работы с буквами применяется последовательный перебор букв в слове.  
...
Потому что методы и классы не должны знать логику работу более высоких слоев программы, у которых могут быть свои планы на тему того, когда и почему нужно завершать работу программы в целом.
...
Использование енама не по назначению- здесь это что-то типа недокласса. Енам должен быть просто хранить константы, иногда довольно сложные, но не содержать в себе прикладную логику.  
...
- Использование енамов состояний ничего хорошего не добавляет программе, а делает ее больше размером и сложнее, алгорим становится запутанее.  
...
Иногда здесь осмысленное ООП подменяется карго культом. Например, бессмысленные в текущей реализации интерфейсы, которые не инжектятся сверху вниз, а инициализируются в самих классах.  
...
Каждая фабрика будет получать слова из одного конкретного источника. Набор слов зависит от выбранного языка, поэтому фабрики в конструктор должны принимать лангуаге
```
...
+ Фабрики слов нужно инициализировать в самом верху, например в слое `main`. Тем самым можно делать разные игровые конфигурации.  
Например(упрощая):
...
```
Вторая конфигурация, язык выбирает юзер, слова грузятся из файла, если ошибка чтения- слова грузятся из памяти
```
public class Main3 {
  public static void main(String[] args) {
...
Нужно разобраться, зачем нужны интерфейсы и как их необходимо использовать в проектах что бы они имели смысл, а не были просто для красоты.  
