...
//ПРАВИЛЬНО:
System.out.println("Текущее состояние слова: " + sb); 
System.out.printf("\nТекущее количество ошибок: %d/%d  ошибок  \n", mistakes, MAX_ERRORS);
```
...
```
В этом пункте ТЗ нет тредований к количеству классов в программе. Есть констатация того, что их физически не может быть меньше чем 1.
...
Сейчас для определения русской буквы используется стока со всеми русскими лиерами:
```
...
Например, смотрим на код метода и видим функционально обособоленные блоки:
```
int counter = 0;
for (int j = 0; j < choice_word.length(); j++) {
  if (sub.charAt(0) == choice_word.charAt(j)) {
