...
Модель, в эта бизнес логика по чтению файла- модель, не должна зависеть от представления и печатать в конкретную среду ввода-вывода, например консоль
```
...
Оно будет перехвачено в более высоких слоях, например в `Main`, там же распечатается соотвествующее сообщение и программа корректно закроется.
...
wordsPool.addAll(Arrays.stream(line.split(":")).toList());
```
Слова в файле должны идти тупо построчно, вот так:
```
король
королева
принц
...
Если класс сделать не утилитным и его методы не статическими, это даст проекту очевидную точку расширения. 
Можно будет в перспективе сделать семество классов , которые будут одинаково работать через полиморфизм- один, как этот, будет печатать на русском, второй- на английском и т.д.
