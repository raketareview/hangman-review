...
- Сеттер должен что-то устанавливать и не должен ничего возвращать.  
Сеттер, который что-то возвращает- нонсенс
```java
void setRandomWord()
...
- Не делай сложных условий в if'ах, это плохо читается.
...
**4. Не бросай исключения в космос**
...
**6. Проверямые исключения перехватывай в точке их возникновения и заменяй на непроверяемые** 
...
Хоть инкапсуляция это термин ООП, тем не менее, в этом классе публичным должен быть только метод main().  
Так и сделано.
