...
- Название должно объяснять, что делает метод. Из названия этого метода невозможно понять, что он делает. Он проверяет, что `Player player` это счетчик попыок?
```
...
**9. public GameLogic()**, олсновная игровая логика
...
В данном случае нет никакой необходимости в том, чтобы этот метод был статичным
```
...
```
На самом деле этот сброс, а по факту начальная инициализация, должен происходить в самом начале метода `void beginningGame()`, перед началом вечного цикла.
...
- Картинки в массиве это ок. Но заполнение массива картинками нужно вынести из конструктора и сделать константой.
...
Конечно, ООП здесь не очень. Но оно тут все-таки есть и для пробы пера приемлемо. Нужно глубже понять принципы, по которым в ООП программе логика делится на классы.
