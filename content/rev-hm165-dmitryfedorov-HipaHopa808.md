...
**5. Создание объекта и использование объекта это две разные операции**
```java
...
```
**Лайфхак:** если идея что-то подчеркивает желтым(здесь- слово "word"), значит она дает какую-то подсказку.  
...
Этот енам используется как эрзац-массив, поэтому лучше, если он и будет массивом:
```java
public enum Hangman {
  FIRST_STATE("""
        +---+
...
Но вообще, если эта реализация претендует на ООП, то нужно картинки и метод их распечатки вынести в отдельный класс.  
...
которая не зависит от представления(или по крайней мере *не должна* от неё зависеть).  
...
Этот метод делает две разные вещи: проверяет состояние игры и печатает сообщение о текущем состоянии игры  
```java
...
Здесь не видно ООП декомпозиции- классы в программе не являются объектами в понимании ООП, а больше похожи на модули процедурного стиля.

## ВЫВОД

При процедурном стиле, нужно научиться писать простые понятные методы без побочных эффектов,
которые будут соблюдать правило одной операции.
