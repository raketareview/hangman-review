...
Извини, но интерфейс вообще никакой.  
...
🔹 Метод может называться так же, как поле класса в редких случаях и только если метод является геттером этого поля. А эти названия похожи до уровня смешения 
```java
