...
- Избыточный контекст. Каждое слово в названии должно как-то лучше понимать суть того, что делает метод. Слово "Raw" здесь ничего не добавляет к пониманию, это слово-паразит.  
...
**6. Не пиши область видимости в интерфейсах**, эта область видимости там всегда `public`
```
...
**7. Приоритет- простота и понятность**. Не пытайся засунуть все в одну строку в ущерб читабельности
```
...
**8. class Letter implements Comparable<Letter>**
...
**10. interface WordReader + class FileWordReader implements WordReader**
...
**13. Интерфейсы ввода-вывода и их консольные реализации**
...
Фактически, сервис для `WordReader`- читает через него слова и выдает одно случайное из них.
